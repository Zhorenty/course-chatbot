...
Админ может заменить PDF гайда у конкретного потока: «Google Sheets» → «Управление курсами» (кнопка на карточке или файл прямо в карточке). В личку без каталога — сохранится как гайд текущего активного запуска. В репозитории уже лежит `assets/guide.pdf` («Гайд Язык цвета»); бот отдаёт его сам, `LEAD_MAGNET_FILE_ID` не обязателен. Картинки воронки из таблицы «Тексты бота» лежат в `assets/funnel/` и уходят вместе с соответствующими сообщениями.
...
- Вкладка **`COURSES`** (`gid=0`) — каталог запуска (обычная цена, спеццена эфира, предоплата, старт курса, эфир, **старт продаж**, конец продаж, канал). Править можно в боте («Google Sheets» → «Управление курсами») или руками: шапка как у воронки, русские колонки, примечания к заголовкам, колонка «статус», одна активная строка. Бот пишет и удаляет строки, вкладку не пересобирает и `gid=0` не удаляет. Если лист пустой, бот один раз запишет текущие значения (19 000 ₽ / спеццена 15 000 ₽ / предоплата 5 000 ₽, эфир 29.09.2026 19:00, старт продаж 30.09.2026, старт курса и конец продаж 12.10.2026). Срок доплаты бот считает сам: за неделю до старта курса. Пустой может остаться только ссылка на эфир. Файл гайда в таблице не хранится — его меняют в карточке курса в боте.
- Вкладка **`ССЫЛКИ`** — диплинки в бота (`t.me/<bot>?start=<метка>`). Править можно в боте («Google Sheets» → «Управление диплинками») или руками: подписи и метки; колонку «Ссылка» заполняет бот. Колонка «Поток» — выпадающий список названий запусков с `COURSES` (пусто = текущий). Лист не пересобирается.
- Вкладка **`ТЕКСТЫ`** — письма ученику (этап, текст, вложение, кнопка). Бот выгружает тексты **один раз**, если лист пустой; дальше не затирает и не читает их как живые письма. Даты и цены в чате берутся из `COURSES`.
- Вкладка **`ВОРОНКА`** — срез воронки. Бот пересобирает её сам; руками не править. Старый лист `FUNNEL` при обновлении удаляется.

Старт бота, кнопка «Обновить Sheets» в «Google Sheets» и вход в «Управление курсами» перечитывают `COURSES` в SQLite. «Обновить Sheets» ещё обновляет ссылки на `ССЫЛКИ` и пересобирает `ВОРОНКА`. Пошагово: [`docs/DEPLOY.md`](docs/DEPLOY.md) → раздел Google Sheets.
