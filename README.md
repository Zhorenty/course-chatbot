...
Админ может заменить PDF гайда у конкретного потока в «Управление курсами» (кнопка на карточке или файл прямо в карточке). В личку без каталога — сохранится как гайд текущего активного запуска. В репозитории уже лежит `assets/guide.pdf` («Гайд Язык цвета»); бот отдаёт его сам, `LEAD_MAGNET_FILE_ID` не обязателен.
...
Старт бота, кнопка админа «Обновить Google Sheets» и вход в «Управление курсами» перечитывают `COURSES` в SQLite. Кнопка Sheets ещё обновляет ссылки на `ССЫЛКИ` и пересобирает `ВОРОНКА`. Пошагово: [`docs/DEPLOY.md`](docs/DEPLOY.md) → раздел Google Sheets.
