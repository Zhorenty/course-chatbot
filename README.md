...
- Вкладка **`COURSES`** (`gid=0`) — каталог запуска (цена, предоплата, даты). Её правят руками: шапка как у воронки, русские колонки, примечания к заголовкам, колонка «статус», одна активная строка. Если лист пустой, бот один раз запишет текущие значения (18 000 ₽ / 5 000 ₽, доплата 05.10.2026, старт 12.10.2026).
- Вкладка **`ССЫЛКИ`** — диплинки в бота (`t.me/<bot>?start=<метка>`). Руками правят подписи и добавляют метки; колонку «Ссылка» заполняет бот. Лист не пересобирается. Кнопка админа «Диплинки» отдаёт те же ссылки в личку.
