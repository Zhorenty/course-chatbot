...
| Отписка | Кнопка «Не писать» в прогреве. Гайд / запись остаются. Напоминания про незакрытый платёж **не** глушить |
...
| Перевыдача invite | Только админ из карточки. Клиент новую ссылку сам не запрашивает |
...
1. `/start` + метка → источник → гайд или курс. Первый экран — **inline** CTA (`guideOfferKeyboard` / карточка курса); reply-меню пишется отдельно как навигация. Повторный `/start` в `lead` открывает тот же экран по **сохранённому** `source`, не по текущему payload.
2. Регистрация = Telegram. Без контакта и email.
3. PDF по `file_id` (админ заливает как onboarding-видео в DVOR) или URL.
4. Сразу текст шага 0 прогрева + кнопки «Записаться» / гайд / «Не писать».
5. `magnet_issued` не завершает воронку. Повторный `/start` в прогреве не открывает оффер заново. «Записаться» **не** ставит `checkout`: фаза меняется, только когда касса отдала URL (`awaiting_payment`).
6. Дальше джоб по таблице шагов с якорем `magnet` / `first_start` (любой `lead` без гайда) / `course_start` (слоты −7/−3/−1 до старта; пропущенный слот не догоняем). Исключить: `warmup_opt_out`, `checkout`, `paid` / `access_granted` / `deposit_paid` / `cancelled`. У `deposit_paid` продающий прогрев **выкл**, напоминания доплаты **вкл** (окно 1–3 дня до срока, в день срока, просрочка один раз). Даты — календарь Europe/Moscow.
7. Брошенная оплата: `checkout_started` и `awaiting_payment`, 6 ч и сутки; за 3 дня до старта — одно касание вместо 6ч/24ч. Invite без входа — 24 ч **или** к старту, не оба в одном тике; в письме текущая ссылка, без кнопки «новая ссылка».
...
Оплата / гайд / отписка — **inline**, `callback_data` ≤ 64 байт, префикс + id. Reply — запись и гайд, пока нет доступа; помощь.
...
Копировать `GoogleSheetsFunnelDashboard`: шапка, KPI-карточки, путь по шагам (% от старта / от предыдущего), 7д/30д, «где сейчас» + «источник». Плюс **конверсия по каждой метке** (Start / гайд / чекаут / купили) и качество базы (opt-out, блок бота, invite без входа). «В воронке» — без paid/access/cancelled. Убрать квиз DVOR. KPI курса: Start, в воронке, взяли гайд, начали оплату, купили / списание, конверсия. Wipe+create листа, входные вкладки не трогать (у курса входных расписаний нет — один bot-owned лист плюс при необходимости «ДЕЙСТВИЯ»).
...
4. Перевыдача **только админом из карточки**: новая ссылка, старую отозвать. Клиент кнопку перевыдачи не видит.
