...
| Отписка | Кнопка «Не писать» в прогреве. Профиль / гайд / запись остаются. Напоминания про незакрытый платёж **не** глушить |
...
Оплата / гайд / отписка — **inline**, `callback_data` ≤ 64 байт, префикс + id. Reply — профиль и помощь; запись и гайд, пока нет доступа.
