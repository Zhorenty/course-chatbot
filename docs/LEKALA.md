...
CTA «Записаться» — пока нет успешной оплаты (`deposit_paid` / `paid` / `access_granted`). После оплаты в reply-меню «Мой курс»: сумма, статус, старт потока, канал. Доплата и invite — inline с этой карточки.
...
1. `/start` + метка → источник → гайд или курс. Первый экран — текст оффера; CTA — **reply-меню** (гайд / запись / помощь), без inline-дублей этих кнопок. Повторный `/start` в `lead` открывает тот же экран по **сохранённому** `source`, не по текущему payload.
2. Регистрация = Telegram. Без контакта и email.
3. PDF по `file_id` (админ заливает как onboarding-видео в DVOR) или URL.
4. Сразу текст шага 0 прогрева + inline «Не писать». Запись и гайд — в reply-меню, не дублировать inline.
...
7. Брошенная оплата: `checkout_started` и `awaiting_payment`, 6 ч и сутки; за 3 дня до старта — одно касание вместо 6ч/24ч. Invite без входа — 24 ч **или** к старту, не оба в одном тике; в письме текущая ссылка и кнопка открыть канал. Без кнопки перевыдачи и без гайд/помощь inline (они в reply-меню).
...
Оплата / отписка / URL канала — **inline**, `callback_data` ≤ 64 байт, префикс + id. Гайд, запись и помощь — только reply-меню, не дублировать теми же кнопками под текстом. После оплаты в reply вместо записи — статус курса.
