...
### Google Sheets (`COURSES` + срез `ВОРОНКА`)

Первый лист таблицы (`gid=0`) — каталог запуска **`COURSES`**. Его правят руками (цена, предоплата, даты). Если он пустой, бот при первом коннекте запишет шапку и текущие значения. Бот пересобирает вкладку `ВОРОНКА` раз в 5 минут и по кнопке админа; `ВОРОНКА` руками не править (wipe + create). Старый лист `FUNNEL` удаляется. `gid=0` бот не удаляет и не затирает дашбордом.
...
Вкладка `ВОРОНКА` появится сама. Руками её не верстать. Первый лист (`gid=0`) бот переименует в `COURSES` и заполнит, если он пустой.
