...
В каждом сообщении воронки (и в профиле) доступна кнопка **«Записаться на курс»**. Прогрев не блокирует запись.

В сообщениях прогрева — кнопка **«Не писать»**. После отписки бот не шлёт прогрев; профиль, гайд и запись остаются. Напоминания про уже начатую оплату / доплату отписка не глушит.
...
4. Может сразу нажать «Записаться» или читать дальше — бот пишет по цепочке, когда она будет в боте. В любой момент открывается **«Профиль»**: поток, статус, гайд, оплата/остаток и канал — и один следующий шаг. **«Записаться»** на клавиатуре, пока нет доступа в канал.
