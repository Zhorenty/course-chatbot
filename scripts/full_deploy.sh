...
#
# Already on the VPS:
#   bash /opt/course-chatbot/scripts/full_deploy.sh
#
# Override SSH target: COURSE_SSH=root@IP ./scripts/full_deploy.sh
set -euo pipefail

REMOTE_DIR="/opt/course-chatbot"
COURSE_SSH="${COURSE_SSH:-course-bot}"
...
echo "==> SSH ${COURSE_SSH}: pull + full rebuild"
exec ssh -t "$COURSE_SSH" "cd ${REMOTE_DIR} && git pull --ff-only && bash scripts/full_deploy.sh"
